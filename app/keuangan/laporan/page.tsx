...
export default function LaporanBulananPage() {
  ...
  const [userWilayah, setUserWilayah] = useState<'Timur' | 'Barat' | null>(null)
  ...
  // Determine user's wilayah based on RT
  // RT 001,002,003 = Timur | RT 004,005,006 = Barat
  useEffect(() => {
    const detectWilayah = async () => {
      if (!userData) return
      
      // Pengurus RW bisa akses semua
      if (isRW) {
        setWilayahLoaded(true)
        return
      }

      let rtId = userData.rt_id // Pengurus RT punya rt_id di users
      
      // Warga biasa: ambil rt_id dari tabel warga
      if (!rtId && userData.warga_id) {
        const { data: wargaData } = await supabase
          .from('warga')
          .select('rt_id')
          .eq('id', userData.warga_id)
          .single()
        rtId = wargaData?.rt_id
      }
      ...
      if (rtId) {
        ...
        if (rtData) {
          const rtNum = parseInt(rtData.nomor_rt)
          const wil = rtNum <= 3 ? 'Timur' : 'Barat'
          setUserWilayah(wil)
          ...
          <div className="row g-3 align-items-end">
            ...
            <div className="col-md-4">
              ...
              <select
                ...
                disabled={!isRW && userWilayah !== null}
              >
                {isRW ? (
                  <>
                  ...
                  <option value={userWilayah || 'Timur'}>Discovery {userWilayah || 'Timur'}</option>
                )}
              </select>
            </div>
          </div>
